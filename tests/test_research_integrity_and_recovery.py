...
def test_unexpected_modbus_and_opcua_transition_are_semantic(tmp_path):
 ev=load('evaluate_sem','experiments/evaluate.py')
 d=tmp_path/'m'; d.mkdir(); (d/'run.json').write_text(json.dumps({'experiment_id':'EXP-UNEXPECTED-MODBUS-SOURCE'})); (d/'conn.log').write_text('#fields\tuid\nX\n'); (d/'conduit-classification.json').write_text(json.dumps([{'classification':'EXPECTED'}])); assert not ev.evaluate(d)['pass']; (d/'conduit-classification.json').write_text(json.dumps([{'classification':'UNEXPECTED'}])); assert ev.evaluate(d)['pass']
 o=tmp_path/'o'; o.mkdir(); (o/'run.json').write_text(json.dumps({'experiment_id':'EXP-OPCUA-STALE-CARGO'})); pcap(o/'cargo-modbus.pcap'); (o/'freshness-before.json').write_text('{"fresh":true}'); (o/'freshness-after.json').write_text('{"fresh":false}'); assert ev.evaluate(o)['pass']
